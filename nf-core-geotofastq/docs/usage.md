# nf-core/geotofastq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/geotofastq/usage](https://nf-co.re/geotofastq/usage)
...
```console
nextflow run nf-core/geotofastq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/geotofastq
```
...
First, go to the [nf-core/geotofastq releases page](https://github.com/nf-core/geotofastq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
